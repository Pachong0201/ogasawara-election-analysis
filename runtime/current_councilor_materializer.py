"""Materialize official 2026 city/county councilor registrations into L3 profiles.

The source is the CEC registration roster. Records describe registration only;
they do not imply qualification approval, incumbency, support strength, or
election outcome.
"""
...
class CurrentCouncilorMaterializer:
    def __init__(
        self,
    ...
    def _question(county: str) -> str:
        return (
            f"2026年{county}議員選舉有哪些已完成登記的候選人，"
            "其選舉區、登記日期與政黨推薦為何？"
        )
    ...
    def _lead(self, county: str, candidate: Dict[str, Any], question: str) -> Dict[str, Any]:
        name = str(candidate.get("name") or "").strip()
        date = str(candidate.get("registration_date") or "").strip()
        district = str(candidate.get("electoral_district") or "").strip()
        party = str(candidate.get("recommended_by_party") or "").strip()
        reference = str(candidate.get("source_reference") or self.adapter.page_url).strip()
        party_text = party or "未由政黨推薦"
        return {
            "lead_id": (
                f"cec-current-councilor-{safe_component(county)}-"
                f"{safe_component(str(candidate.get('candidate_id') or name))}"
            ),
            "county": county,
            "query": question,
            "research_questions": [question],
            "title": "115年地方公職人員選舉議員候選人登記名冊",
            "summary": (
                f"中央選舉委員會登記名冊列示{name}於{date}完成{district}"
                f"議員候選人登記；推薦政黨欄為{party_text}。"
            ),
            "evidence": f"{district}；{date}；{name}；推薦之政黨：{party_text}",
            "url": reference,
            ...
            "retrieved_at": str(candidate.get("last_verified_at") or utc_now_iso()),
        }
    ...
    def _proposal(
        ...
        last_verified = str(candidate.get("last_verified_at") or utc_now_iso())[:10]
        district = str(candidate.get("electoral_district") or "").strip()
        return {
            ...
            "contradiction_check_note": (
                "重新讀取中選會115年9月7日議員候選人登記名冊並以當次官方頁面為"
                "登記狀態基準；僅保存完成登記事實。後續撤回、資格審定或名單異動"
                "須另行更新。"
            ),
            "scope_boundary": (
                "僅確認115年地方公職人員選舉議員候選人的選舉區、姓名、登記日期"
                "與政黨推薦欄；不得推斷現任身分、資格審定結果、支持度、動員效果"
                "或選舉結果。"
            ),
            "target_record": {
                ...
                "election_year": 2026,
                "jurisdiction": county,
                "official_jurisdiction": str(candidate.get("official_jurisdiction") or county),
                "electoral_district": district,
                "candidate_status": "registered",
                "registration_date": str(candidate.get("registration_date") or ""),
                "recommended_by_party": str(candidate.get("recommended_by_party") or ""),
    ...
    def materialize_county(self, county: str, *, apply: bool = False) -> Dict[str, Any]:
        ...
        fetched = self.adapter.fetch(county, "councilor", 2026)
        ...
        for candidate, lead in zip(records, leads):
            result = self.builder.promote(
                self._proposal(county, candidate, lead, question),
                dry_run=not apply,
                build_package=False,
            )
    ...
    def materialize_many(
        ...
        *,
        ...
        manifest = self.repo_root / "data" / "manifests" / "l3_official_councilors_latest.json"
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    result = CurrentCouncilorMaterializer(args.repo_root).materialize_many(
        _parse_counties(args.counties), apply=args.apply
    )
